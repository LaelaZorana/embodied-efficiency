# Everyone Can Train a VLA. Almost Nobody Can Run One on the Robot.

*Why efficiency, not capability, is the real bottleneck in embodied AI, and what I am building at that edge.*

---

A vision-language-action model can fold laundry, sort parts, and follow a spoken instruction in a lab demo today. Then you put it on the actual robot and it falls apart, and the reason is not that it cannot do the task. It cannot do it fast enough. End-to-end VLA inference runs at 3 to 5 Hz on a research GPU, and a robot arm needs 50 to 100 Hz to move smoothly. The capability is sitting right there. What is missing is everything that gets it onto the robot.

The frontier of embodied AI is no longer whether the model can do it. It is whether the model can do it on this robot, under this latency budget, inside this power envelope, cheaply enough to deploy a thousand of them. That is an efficiency problem, kernels and quantization and scheduling and compilation, and it is the least glamorous and most under-staffed layer in the whole stack.
...
The ideas are not the missing piece. The engineering is: making those techniques reproducible, vendor-neutral, and deployable on the hardware robots actually carry, Jetson Orin and Thor, custom silicon running at single-digit watts, without losing the fine dexterity that made the model worth deploying in the first place.
...
## How I got here, which was not an accident

I came at this sideways. For the last cycle I have been golfing neural networks, building the smallest ONNX graphs that still solve ARC-AGI reasoning tasks exactly, scored against a hard budget of parameters and memory. It sounds like a puzzle. It is the same problem.

Fitting a correct solution under a hard size budget and fitting a VLA under a hard latency budget are the same search, the same instinct for where precision comes free and where it carries the load. I spent months sharpening that instinct against a scorer that gave nothing away. I am pointing it at robots now.
...
Embodied AI's headlines are about capability. The bottleneck is efficiency, and a trust layer is what makes that efficiency safe to deploy. That intersection, kernels and quantization for physical AI with safety built in, is where I am putting my work.

If you are building robots and fighting the deploy gap, or working the same edge from the lab side, I would like to compare notes.
